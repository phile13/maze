class gamespace{
  constructor(board, id, type, x, y){
    ...
    this.boardCenter = { x : this.boardScale * this.boardSize.width / 2 , y : this.boardScale * this.boardSize.height / 2 };
  ...
  MOVE(msg){
    if(msg.ID >= 0){
      ...
      if(msg.ID == this.myid){
        this.app.stage.x = this.canvasCenter.x - this.boardCenter.x + (who.x - this.boardCenter.x);
        this.app.stage.y = this.canvasCenter.y - this.boardCenter.y + (who.y - this.boardCenter.y);
      }
    }
  }
